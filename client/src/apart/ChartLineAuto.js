...
import common from '../include/common/common_js'
...
const getDatas = async (props) => {
  try {
    const response = await axios.post(
      ...
      {아파트:props.아파트,법정동:props.법정동,start_dt : props.start_dt,end_dt : props.end_dt}
    );
    var labelArr = [];
    var dataAVGArr = [];
    var dataMINArr = [];
...
function ChartLineAuto(match) {
  ...
  var 아파트="";
  var 법정동="";
  var start_dt = common.getDateBasic(new Date(new Date().getTime() - 6 * 30 * 24 * 60 * 60 * 1000));
  var end_dt = common.getDateBasic(new Date());
  if(match.match.params.apartName!=undefined){
    아파트 = match.match.params.apartName.split('__')[0];
    법정동 = match.match.params.apartName.split('__')[1];
  }
  var props={아파트:아파트,법정동:법정동,start_dt:start_dt,end_dt:end_dt};
  console.log(props)
  ...
  async function sendChartMail(){
    ...
    const response = await axios.post(
      ...
      {href:chartRef.current.toBase64Image(),아파트:props.아파트,법정동:props.법정동,start_dt :props.start_dt,end_dt : props.end_dt}
    );
  }
