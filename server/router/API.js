...
router.get('/apart/getMainAddr',function(req,res){
    AddrModel.aggregate([
        // {$match:{"시군구명":"12314"}},
...
router.get('/apart/getHealthCheck',async function(req,res){
...
router.post('/apart/getAPIData_ELS',async function(req,res){
    console.log("---------------------ELS활용--------------------------")
    ...
    var count = await callELSTotCnt(req,'apis/_count',query);
    ...
    if(skip+limit>300000){
        ...
        console.log("쿼리 ~~~")
        console.log(query)
        await APIModel.find(query, project,  function(err, addrs){    
    ...
    //검색조건들 추가해줘야해.
        await callELS(req,'apis/_search',query).then(result => {
...
function callELS(req,url,params){
    ...
    return new Promise(async function(resolve, reject){
        ...
        var matchFlag = false;
        var match = {};
        // console.log(params.아파트)
        if(params.아파트!=undefined){
            matchFlag = true;
            console.log("단어검색")
            match = {"아파트" : params.아파트.$regex};
